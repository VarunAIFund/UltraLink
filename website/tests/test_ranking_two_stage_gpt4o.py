"""
Test Two-Stage Ranking Pipeline (GPT-5-nano → GPT-4o with hardcoded indices)
Tests the new GPT-4o ranking with structured outputs that enforce all indices
"""
import sys
import os
import json
...
def estimate_cost(num_candidates, num_strong):
    """
    Estimate cost for two-stage pipeline with GPT-4o

    Stage 1 (GPT-5-nano): $0.00075829383 per candidate (measured)
    Stage 2 (GPT-4o): ~$0.20 per 100 strong candidates with structured outputs
    """
    ...
    # Stage 2: GPT-4o ranking (strong matches only, hardcoded indices)
    # Estimated at ~$0.002 per strong candidate (higher than Gemini but more reliable)
    STAGE_2_COST_PER_STRONG = 0.002
    stage_2_cost = num_strong * STAGE_2_COST_PER_STRONG
...
async def test_two_stage_pipeline(query: str, connected_to: str = 'all', limit: int = None):
    """
    Test the complete two-stage ranking pipeline with GPT-4o
    ...
    """
    print(f"\n{'='*80}")
    print(f"TWO-STAGE PIPELINE TEST (GPT-5-nano → GPT-4o)")
    print(f"{'='*80}")
    print(f"Query: {query}")
    ...
    print(f"   Strong: {num_strong}, Partial: {num_partial}, No match: {num_no_match}")
    print(f"   Time: {stage_1_time:.2f}s")
    print(f"   Rate: {len(candidates)/stage_1_time:.1f} candidates/second\n")

    # Save Stage 1 results immediately (before Stage 2)
    stage_1_file = f"stage_1_results_{len(candidates)}_candidates_gpt4o.json"
    stage_1_data = {
        'query': query,
    ...
    # Step 3: Stage 2 - GPT-4o Ranking + Rule Scoring
    print("STEP 3: Stage 2 Ranking & Scoring (GPT-4o with hardcoded indices)...")
    start_stage_2 = time.time()
    final_results = await rank_all_candidates(query, stage_1_results)
    stage_2_time = time.time() - start_stage_2

    print(f"   Time: {stage_2_time:.2f}s")

    # Check if GPT-4o ranked all strong matches (using LinkedIn URLs as unique identifiers)
    ...
    if len(missing_urls) == 0 and len(extra_urls) == 0:
        ...
        print(f"      ✅ Hardcoded indices enforcement: 100% success!")
    else:
        if missing_urls:
            print(f"      ❌ Missing: {len(missing_urls)} candidates GPT-4o skipped")
        ...
        if extra_urls:
            print(f"      ❌ Extra: {len(extra_urls)} candidates not in input")
    ...
    # Save Stage 2 results (ONLY GPT-4o-ranked strong matches)
    stage_2_file = f"stage_2_results_{len(candidates)}_candidates_gpt4o.json"
    stage_2_data = {
        'query': query,
        'stage': 'Stage 2A: GPT-4o Ranking (Strong Matches Only)',
        'note': 'This file only contains strong matches processed by GPT-4o with hardcoded index fields. Partial/no_match candidates are in the combined results file.',
        'time': stage_2_time,
        'gpt4o_completeness': {
    ...
    output_file = f"two_stage_results_{len(candidates)}_candidates_gpt4o.json"
    output_data = {
        'query': query,
...
async def run_all_tests():
    ...
    print("\n" + "="*80)
    print("TWO-STAGE RANKING PIPELINE - GPT-4o TEST SUITE")
    print("="*80)
